...
fastify.get('/api/status', async (request) => {
  ...
  const national = statutory ?? (nationalRow
    ? { status: nationalRow.status, reason: nationalRow.reason, source: nationalRow.source, since: nationalRow.since }
    : { status: 'full', reason: null, source: null, since: null });

  const stateStatus = stateRow
    ? { status: stateRow.status, reason: stateRow.reason, source: stateRow.source, since: stateRow.since }
    : { status: 'full', reason: null, source: null, since: null };
